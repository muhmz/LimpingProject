...
%% Load Data
app2.enable.HighpassEqui = 0;   % remove high freq comp from raw data
...
%% range profile segments
    ...
    if(SeeAllRanges)
        ...
        app2.R1_stop_m = 9.6*2; app2.R1_start_m = 3e8/2/app2.bw;
...
%         RngDopp_psd_dB_interf = 10*log10(RngDopp_psd_interf);
        ...
        %% Doppler PSD
        if ch_i == 1
            Dopp_psd_dB1(ii_data-1,:) = 10*log10(sum(RngDopp_psd_1)) + app2.dBGain1;
            Dopp_psd_dB3(ii_data-1,:) = 10*log10(sum(RngDopp_psd_interf)) + app2.dBGain3;
            
        elseif ch_i == 2
            Dopp_psd_dB2(ii_data-1,:) = 10*log10(sum(RngDopp_psd_1)) + app2.dBGain2;
        end
...
data_mat = reshape(dataVec, app2.nSamp, []);
